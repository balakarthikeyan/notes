...
## Q2 - What is explicit and implicit type assignment?

- `Explicit` means writing out the type. `let firstName: string = "Joe";`

- `Implicit` means TypeScript will guess the type, based on the value. Like below type will be considered a number. `let age = 41;`

## Q3 - Difference between any, unknown and never in TypeScript?

The type of any is used to assign any type of a variable. It will not give error even if you reassign another type.
```js
let x: any = 10;
...
```
The type never represents value that never occurs. It is typically used for return statements of function that doesn't returns properly.
```js
function throwError(message: string): never {
    throw new Error(message);
}
```
## Q4 - How do you give the type of Arrays?
...
names.push("Bala"); // no error
```
We can also use a readonly keyword, which prevents the array been changed.
```js
const names: readonly string[] = ["Joe", "Ram", "Rahul"];
names.push("Jack"); // Error: Property 'push' does not exist on type 'readonly string[]'.
```
## Q5 - What is Type Inference in array?

If we don't give any type to an array, it will infer the type.
```js
...
```
## Q8 - How to give the types for Objects?
...
```
## Q9 - How to have optional properties in Objects?
...
```
## Q10 - Explain enum in TypeScript?
...
```
## Q15 - What are Union types?

Union types are used when the property can be more then one value, like string or number. For this reason, they are also called OR and are used by using | symbol.
```js
...
printSucessCode('200');
```
## Q16 - How to give the return type in function?
...
```
## Q17 - How to give type of parameters in function?
...
```
## Q18 - How to give optional, default and rest parameters in function?

With default parameter, we can mark a parameter as optional denoted by ?.
```js
...
```
## Q19 - What is casting in TypeScript?
...
```
## Q20 - How to give type of a variable in Class?

We have given the type of name as string in a class.
```js
class Developer {
  name: string;
}
...
dev.name = "Joe";
```
## Q21 - What is public, private and protected in TypeScript classes?
...
// shorthand version, by directly adding the private variable in the constructor.
...
// Example of protected, we need to inherit from a base class, which we do with the extends keyword. Also, we can get data from an interface to a class, using the implements keyword.
...
// Logs 400
```
## Q22 - What is the readonly keyword in reference to classes in TypeScript?
...
console.log(person.getName());
```
## Q23 - How to implement overriding in classes of TypeScript?
...
```
## Q25 - What are Singleton classes.
...
//Logs - true
```
## Q26 - What are Generics in TypeScript. Give examples in functions, classes and type aliases.
...
const value = new GenericClass<number>('theNum');
value.setValue(10);
...
```
## Q27 - What is Partial, utility type.
...
console.log(rectPart); 
// Logs - { width: 100 }
```
## Q28 - What is Required, utility type.

Required changes all the properties to be required in an object. In the below example, the mileage is made optional in the interface. But when we had made it Required while creating the object, we are forced to give the mileage.
```js
interface Car {
  make: string;
  model: string;
...
console.log(myCar); 
// Logs - { make: 'Tata', model: 'Tiago', mileage: 20 }
```
## Q29 - What is Record, utility type.
...
// Logs - { name: 'Joe' }
```
## Q32 - What is Exclude, utility type.
...
```
## Q33 - What is Readonly, utility type.
...
```
## Q34 - What is Nullish Coalescence.
